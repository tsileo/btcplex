...
func (tx *Tx) Revert2(spool *redis.Pool) (err error) {
	ssdb := spool.Get()
	defer ssdb.Close()
	...
	if !done {
		...
		if tx.BlockHash == txupdated.BlockHash {
			...
			for _, addr := range tx.Addresses() {
				ssdb.Do("ZADD", fmt.Sprintf("addr:%v", addr), tx.BlockTime, tx.Hash)

				ssdb.Do("ZADD", fmt.Sprintf("addr:%v:sent", addr), tx.BlockTime, tx.Hash)
			}
		}
